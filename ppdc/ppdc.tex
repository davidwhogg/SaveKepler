...
Inspired by ideas from causal inference,
  ...
  is predicted using a weighted linear combination of many other pixels
...
We learn also a second model that uses,
  in addition to spatially separated pixels,
  cospatial pixels but separated by time lags longer than expected transit signals.
These latter models remove stellar variability signals along with spacecraft-induced signals;
