...
While it is obvious \emph{in principle} that we can infer
  ...
  \footnote{Indeed inference of non-trivial intra-exposure
    ...
    case of single-exposure natural images (\citealt{fergus2006,whyte2010,hirsch2011}).
...
Motivated by what we call the ``371-roll scenario''%
  \footnote{More-or-less the most stable pointing option according to
...
An aperture is examined around each target,
...
This is estimated simply by taking
  the geometric mean of the ratio of the model to simulated images.
...
As when producing the simulations,
  the model images are rendered at twice the resolution and scale by a model (parameterized) flat-field
...
In this version, the the best-fit flat-field is determined by iteratively solving a linearized least-squares problem:
...
To avoid numerical overfitting, we added a smoothness prior on the flat-field values,
...
\item The PSF in Kepler's two-wheel era can be decomposed in two
  ...
  computer vision community (\cite{fergus2006,whyte2010,hirsch2011}).
...
If the science program decides to stay in the \Kepler\ field
  (which is consistent with our philosophy)
  then exopsure times will have to get shorter.
...
From a scientific perspective, we much prefer a change to 
...
\begin{figure}
\includegraphics[trim=0.75in 0.25in 0.75in 0.25in, width=0.49\textwidth]{sim2-103.png}%
\includegraphics[trim=0.75in 0.00in 0.75in 0.00in, width=0.49\textwidth]{sim2-200a.png}\\[0.3in]
\includegraphics[trim=0.5in 0.5in 0.5in 0.5in, width=0.49\textwidth]{sim2-200.png}%
\includegraphics[trim=0.5in 0.5in 0.5in 0.5in, width=0.49\textwidth]{sim2-201.png}\\[0.4in]
\includegraphics[trim=0.5in 0.5in 0.5in 0.5in, width=0.49\textwidth]{sim2-202.png}
