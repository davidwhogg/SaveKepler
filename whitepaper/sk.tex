...
However, the fact that the telescope and camera were so stable
  ...
  implies that must be possible to build a deterministic, predictive model
...
While it is obvious \emph{in principle} that we can infer
  ...
  \footnote{Indeed inference of non-trivial intra-exposure
    camera pointing inference has been demonstrated in the far more difficult
    case of single-exposure natural images (HOGG CITE CAMERA SHAKE PAPERS).
  That is, the three-axis history of camera
    has been inferred without even multiple exposures to use
...
That is, 
  ...
  for the determination or inference of calibration parameters.
