...
be captured with a more powerful model. This will be done using *Deep
Learning* approaches that have recently proven very successful for
complex image domain tasks such as object recognition [Kriz12].
...
We intend to adapt this model our application in the following ways:

1. Spatial-only: In this model, we just attempt to model the spatial
...
2. Temporal-only: The drawback to the spatial-only model is that it
...
3. Spatio-temporal: This combines the above models and allows spatial
...
Removing camera shake from a single image, ACM Transactions on Graphics (SIGGRAPH) 2006.
\end{thebibliography}
