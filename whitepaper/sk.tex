...
This is implemented by building the model of the systematics in pixel $i$ at
time $t_j$ using the fluxes of the training pixels $k \ne i$ in a sliding
window $t_j - \delta t \le t \le t_j + \delta t$.
We can further improve the predictive power of the model by including data
from the target pixel ugh.

\paragraph{Terminology}
We are trying to calibrate pixel $i$ of target star $n$ at time $t_k$ using
the pixels $j$ on target $m \ne n$ in the sliding window $t_k - \delta t \le t
\le t_k + \delta t$.
...
$\Delta t \gg $ the timescale of the signal of interest (a transit).
